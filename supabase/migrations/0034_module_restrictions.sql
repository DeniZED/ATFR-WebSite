-- ATFR — Restrictions de modules par utilisateur.
--
-- Complète 0032/0033 : un super_admin peut maintenant aussi RETIRER à un
-- utilisateur un module que son rôle lui donnerait normalement
-- (`user_roles.module_restrictions`). Une restriction ne retire que les
-- droits liés à ce module précis ; `module_access` (accordé) reste
-- prioritaire si les deux sont présents pour la même clé.
--
-- public.module_restricted(key) : true si le module `key` est listé dans
-- module_restrictions pour l'utilisateur courant.

alter table public.user_roles
  add column if not exists module_restrictions text[] not null default '{}';

-- list_users_with_roles renvoie maintenant aussi module_restrictions.
...
as $$
  select u.id, u.email, u.created_at, ur.role,
         coalesce(ur.module_access, '{}'),
         coalesce(ur.module_restrictions, '{}')
  from auth.users u
...
create or replace function public.module_restricted(module_key text)
returns boolean
language sql
stable
security definer
...
as $$
  select coalesce(
    (select module_key = any(module_restrictions) from public.user_roles where user_id = auth.uid()),
    false
  )
$$;

grant execute on function public.module_restricted(text) to authenticated;
...
create policy "moderator can read applications"
  ...
  to authenticated
  using ((public.is_moderator() and not public.module_restricted('candidatures')) or public.has_module('candidatures'));

create policy "moderator can update applications"
  on public.applications for update
  to authenticated
  using ((public.is_moderator() and not public.module_restricted('candidatures')) or public.has_module('candidatures'))
  with check ((public.is_moderator() and not public.module_restricted('candidatures')) or public.has_module('candidatures'));

create policy "admin can delete applications"
  on public.applications for delete
  to authenticated
  using ((public.is_admin() and not public.module_restricted('candidatures')) or public.has_module('candidatures'));
...
create policy "admin can write events"
  ...
  to authenticated
  using ((public.is_admin() and not public.module_restricted('evenements')) or public.has_module('evenements'))
  with check ((public.is_admin() and not public.module_restricted('evenements')) or public.has_module('evenements'));
...
create policy "admin can write members"
  ...
  to authenticated
  using ((public.is_admin() and not public.module_restricted('membres')) or public.has_module('membres'))
  with check ((public.is_admin() and not public.module_restricted('membres')) or public.has_module('membres'));
...
create policy "auth can manage players"
  ...
  to authenticated
  using ((public.is_admin() and not public.module_restricted('rh')) or public.has_module('rh'))
  with check ((public.is_admin() and not public.module_restricted('rh')) or public.has_module('rh'));
...
create policy "editor can write site_content"
  ...
  to authenticated
  using ((public.is_editor() and not public.module_restricted('contenu')) or public.has_module('contenu'))
  with check ((public.is_editor() and not public.module_restricted('contenu')) or public.has_module('contenu'));
...
create policy "editor can write media_assets"
  ...
  to authenticated
  using ((public.is_editor() and not public.module_restricted('galerie')) or public.has_module('galerie'))
  with check ((public.is_editor() and not public.module_restricted('galerie')) or public.has_module('galerie'));
...
create policy "public-media insert"
  ...
  to authenticated
  with check (bucket_id = 'public-media' and ((public.is_editor() and not public.module_restricted('galerie')) or public.has_module('galerie')));

create policy "public-media update"
  on storage.objects for update
  to authenticated
  using (bucket_id = 'public-media' and ((public.is_editor() and not public.module_restricted('galerie')) or public.has_module('galerie')))
  with check (bucket_id = 'public-media' and ((public.is_editor() and not public.module_restricted('galerie')) or public.has_module('galerie')));
...
create policy "editor can write highlights"
  ...
  to authenticated
  using ((public.is_editor() and not public.module_restricted('moments')) or public.has_module('moments'))
  with check ((public.is_editor() and not public.module_restricted('moments')) or public.has_module('moments'));
...
create policy "editor can write achievements"
  ...
  to authenticated
  using ((public.is_editor() and not public.module_restricted('palmares')) or public.has_module('palmares'))
  with check ((public.is_editor() and not public.module_restricted('palmares')) or public.has_module('palmares'));
...
create policy "editor can write testimonials"
  ...
  to authenticated
  using ((public.is_editor() and not public.module_restricted('temoignages')) or public.has_module('temoignages'))
  with check ((public.is_editor() and not public.module_restricted('temoignages')) or public.has_module('temoignages'));
...
create policy "modules write"
  ...
  to authenticated
  using ((public.is_editor() and not public.module_restricted('modules')) or public.has_module('modules'))
  with check ((public.is_editor() and not public.module_restricted('modules')) or public.has_module('modules'));
...
create policy "module_scores moderate"
  ...
  to authenticated
  using ((public.is_moderator() and not public.module_restricted('academie')) or public.has_module('academie'));
...
create policy "quiz_categories write"
  ...
  using ((public.is_editor() and not public.module_restricted('quiz')) or public.has_module('quiz'))
  with check ((public.is_editor() and not public.module_restricted('quiz')) or public.has_module('quiz'));
...
create policy "quiz_sessions update own"
  ...
  to authenticated
  using (user_id = auth.uid() or (public.is_editor() and not public.module_restricted('quiz')) or public.has_module('quiz'))
  with check (user_id = auth.uid() or (public.is_editor() and not public.module_restricted('quiz')) or public.has_module('quiz'));
...
create policy "wot_maps write"
  ...
  using ((public.is_editor() and not public.module_restricted('geoguesser')) or public.has_module('geoguesser'))
  with check ((public.is_editor() and not public.module_restricted('geoguesser')) or public.has_module('geoguesser'));
...
declare
  affected integer;
begin
  if not ((coalesce(public.is_editor(), false) and not public.module_restricted('geoguesser')) or public.has_module('geoguesser')) then
...
create policy "editor can write clan_pages"
  ...
  to authenticated
  using ((public.is_editor() and not public.module_restricted('pages-clan')) or public.has_module('pages-clan'))
  with check ((public.is_editor() and not public.module_restricted('pages-clan')) or public.has_module('pages-clan'));
...
begin
  if not ((public.is_admin() and not public.module_restricted('rh')) or public.has_module('rh')) then
