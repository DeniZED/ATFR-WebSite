...
| ⚠️ P1-5 | Logique métier dispersée dans 11 emplacements UI — **LARGEMENT CORRIGÉ** (tranche 1 : `features/geoguesser/badge.ts` — tiers/étoiles/couleurs de l'insigne, `features/geoguesser/personalStats.ts` — agrégation + `parseMode`, `features/rh/topPerformers.ts` — classements top 5, chacun avec tests ; items 3/9 déjà couverts par P1-4, item 6 supprimé par P0-1. Restent : les 2 systèmes de tier divergents — décision produit sur l'harmonisation des barèmes —, et les fonctions locales de `Geoguesser.tsx` à déplacer lors du découpage P2-1) | Voir `AUDIT_ARCHITECTURE.md` §6 | Non testable, source de divergences (ex. 2 systèmes de tier) | Extraction progressive vers `features/*/logic.ts`, suivre le pattern `features/rh/activity.ts` | Moyen | Élevé | Élevé |
