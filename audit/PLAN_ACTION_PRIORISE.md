...
| ✅ P2-3 | Absence de système de toast/notification unifié — **CORRIGÉ** (`useToast` + `ToastProvider` montés dans `AdminLayout` : pile bas-droite, auto-effacement, `aria-live` ; adoption pilote sur l'éditeur de contenu clan, généralisation progressive aux autres pages admin au fil de l'eau) | Transverse | Introduire un `Toaster` centralisé | Moyen | Élevé |
