...
| ✅ P1-5 | Logique métier dispersée dans 11 emplacements UI — **CORRIGÉ** (tranche 1 : `features/geoguesser/badge.ts` — tiers/étoiles/couleurs de l'insigne, `features/geoguesser/personalStats.ts` — agrégation + `parseMode`, `features/rh/topPerformers.ts` — classements top 5, chacun avec tests ; items 3/9 déjà couverts par P1-4, item 6 supprimé par P0-1 ; fonctions locales de `Geoguesser.tsx` extraites via P2-1 vers `features/geoguesser/mode.ts` et `features/geoguesser/resultStats.ts`. Reste uniquement l'harmonisation des 2 barèmes de tier — décision produit/gameplay, hors périmètre technique) | Voir `AUDIT_ARCHITECTURE.md` §6 | Non testable, source de divergences (ex. 2 systèmes de tier) | Extraction progressive vers `features/*/logic.ts`, suivre le pattern `features/rh/activity.ts` | Moyen | Élevé | Élevé |
| ✅ P1-6 | Contraste insuffisant (94 occurrences `text-atfr-fog/opacité`) — **CORRIGÉ** (107 occurrences `/25` à `/75` relevées à `/85` dans 26 fichiers — sur les fonds du site, `/85` ≈ 5:1 ✓ AA alors que `/70` ≈ 4.1:1 échouait ; les `/80`, ≈ 4.9:1, sont conservés) | `src/pages/clan/*.tsx`, composants Geoguesser | WCAG 1.4.3, lisibilité dégradée | Relever les opacités sous le seuil 4.5:1 | Faible | Moyen | Moyen |
...
|---|---|---|---|---|---|
| ✅ P2-1 | `Geoguesser.tsx` (3317 lignes) — monolithe, re-renders excessifs — **CORRIGÉ** en 2 tranches : logique de modes → `features/geoguesser/mode.ts`, stats de résultats → `features/geoguesser/resultStats.ts`, sous-composants → `components/geoguesser/panels.tsx` ; la page passe à ~1 090 lignes (composant principal + orchestration de session), refactor pur à comportement identique | `src/pages/modules/Geoguesser.tsx` | Découpage en sous-composants/hooks dédiés | Élevé | Moyen-élevé |
| ✅ P2-2 | 8 pages admin CRUD dupliquées + `window.confirm()` ×19 — **CORRIGÉ** : (a) hook `useConfirm` + `ConfirmProvider` (sur base `ModalShell`) et remplacement des 22 `confirm()` natifs de 18 pages admin ; (b) pied de formulaire partagé `FormActions` (Annuler + Enregistrer avec état d'attente) remplaçant le bloc dupliqué dans les 8 formulaires CRUD (`AdminEvents`, `AdminHighlights`, `AdminAchievements`, `AdminTestimonials`, `AdminQuizCategories`, `AdminGeoMaps`, `AdminCwEvents`, `AdminClanPages`). | `src/pages/admin/Admin*.tsx` | Composant `ConfirmDialog`/hook `useConfirm`, factorisation du pattern modale | Moyen | Élevé (gain sur 8+19 emplacements) |
| ✅ P2-3 | Absence de système de toast/notification unifié — **CORRIGÉ** (`useToast` + `ToastProvider` : pile bas-droite, auto-effacement, `aria-live`. Généralisé : provider monté à la racine de l'app + `MutationCache` global React Query — échec de mutation → toast d'erreur traduit par défaut (`meta.silentError` pour les pages qui affichent déjà l'erreur en ligne), succès → `meta.successToast` déclaré sur les ~40 mutations CRUD admin) | Transverse | Introduire un `Toaster` centralisé | Moyen | Élevé |
...
| ✅ P2-5 | Chunk `recharts`/`PieChart` (115 kB gzip) potentiellement sur-chargé — **CORRIGÉ** (graphiques extraits/wrappés en `React.lazy` : `CwEventCharts` pour le détail de campagne CW, `HrTrendChart`/`HrStatusBreakdown` pour le dashboard admin ; le cœur recharts n'est plus chargé qu'en `import()` dynamique au rendu des graphiques, plus au chargement des pages) | Pages avec graphiques | Lazy-load au niveau du sous-composant graphique | Faible | Moyen |
| ✅ P2-6 | Révocation d'accès clan-hub non instantanée (cache 5 min) — **CORRIGÉ** (`staleTime` 5 min → 60 s côté UI ; la barrière réelle est de toute façon serveur depuis P0-2 : `clan-content.mts` re-vérifie le clan à chaque requête) | `src/features/clan/useClanAccess.ts:34` | Réduire le `staleTime` ou ajouter un abonnement realtime si besoin d'instantanéité | Faible | Faible-moyen |
...
| ✅ P2-8 | Métadonnées SEO statiques (identiques sur toutes les routes) — **CORRIGÉ** (`react-helmet-async` + composant `PageMeta` — title/description/OG/Twitter par route — appliqué à Accueil, Recrutement, Événements, Membres) | `index.html`, absence de gestion par route | Introduire `react-helmet-async` sur les pages publiques clés | Moyen | Moyen |
| ✅ P2-9 | Pas de script `gen:types` — risque de dérive de `database.ts` — **CORRIGÉ** (`npm run gen:types` génère `src/types/database.gen.ts` via la CLI Supabase — fichier gitignoré, à differ manuellement contre le `database.ts` maintenu à la main qui porte des types métier custom) | `package.json` | Ajouter le script de génération | Faible | Moyen (préventif) |
